...
```{r}
tmp_preds = predict(svy_glm_res, test_df_count, type = "response" )

test_df_count_tmp = mutate(test_df_count, glm_bin_pred = tmp_preds)

test_df_count_tmp |> 
  mutate(primary_mechanism_bin = ifelse(primary_mechanism == "Multi-sport", "Multi-sport", "Other" )) |>
  ggplot(aes(glm_bin_pred, fill =  primary_mechanism_bin  ), weights = weight ) +
  geom_density(alpha = .2)

test_df_count_tmp |> 
  mutate(primary_mechanism_bin = ifelse(primary_mechanism == "Multi-sport", "Multi-sport", "Other" )) |>
  ggplot(aes(glm_bin_pred, fill =  primary_mechanism_bin  ), weights = weight ) +
  geom_histogram() + facet_grid(.~primary_mechanism_bin)
```


```{r}
train_df_count |>
  count(primary_mechanism)

mutate(train_df_count, treatment_year = year(treatment_date) , primary_mechanism2 = as.numeric(primary_mechanism) )  |>
  count(primary_mechanism2)
```
